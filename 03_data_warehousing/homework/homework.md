...
FROM `ny_taxi.yellow_taxidata_2024`;
```
Estimated Bytes to read: 0 bytes

Because COUNT(*) doesn’t need to scan any data columns. BigQuery can satisfy this from table/partition metadata (row-count statistics) for a materialized or regular table.
